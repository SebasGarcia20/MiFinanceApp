...
export default function Debts({
  ...
  /** Debt id → payment list expanded (collapsed by default to save vertical space) */
  ...
  const handleAddDebt = async () => {
    ...
    try {
      ...
      reportError(err, { t, toast, context: 'Add debt' });
    }
  };
  ...
  const handleAddPayment = async (debtId: string) => {
    ...
    try {
      ...
      reportError(err, { t, toast, context: 'Add payment' });
    }
  };
  ...
  const handleUpdateTotal = async (debtId: string) => {
    ...
    try {
      ...
      reportError(err, { t, toast, context: 'Update debt total' });
    }
  };
  ...
  const handleUpdateName = async (debtId: string) => {
    ...
    try {
      ...
      reportError(err, { t, toast, context: 'Update debt name' });
    }
  };
  ...
  const handleDeleteDebt = async (id: string) => {
    try {
      ...
      reportError(err, { t, toast, context: 'Delete debt' });
    }
  };

  const handleDeletePayment = async (debtId: string, paymentId: string) => {
    try {
      await onDeletePayment(debtId, paymentId);
    } catch (err) {
      reportError(err, { t, toast, context: 'Delete payment' });
    }
  };
  ...
  const { activeDebts, paidOffDebts } = useMemo(() => {
    ...
    for (const d of debts) {
      ...
      const remaining = Math.max(0, d.totalAmount - totalPaid);
      if (remaining === 0) history.push(d);
      else active.push(d);
    }
    return { activeDebts: active, paidOffDebts: history };
  }, [debts]);

  const renderDebtCard = (debt: Debt, variant: 'active' | 'history' = 'active') => {
      ...
      >
              ...
              <div className="space-y-2 mb-2">
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    autoFocus
                  />
                  <button type="button" onClick={() => handleUpdateName(debt.id)} className="btn-success flex-shrink-0">
                    {t('common.save')}
                  </button>
                  <button
                    type="button"
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    autoFocus
                  />
                  <button type="button" onClick={() => handleUpdateTotal(debt.id)} className="btn-success flex-shrink-0">
                    {t('common.save')}
                  </button>
                  <button
                    type="button"
          ...
          <div className="mb-4 p-3 sm:p-4 bg-gradient-to-br from-primary-50 to-white rounded-xl border-2 border-primary-200">
            ...
            <div className="flex flex-col sm:flex-row gap-2">
              <input
                ...
                autoFocus
              />
              <button type="button" onClick={() => handleAddPayment(debt.id)} className="btn-success flex-1 sm:flex-initial min-h-[44px] sm:min-h-0">
                {t('debts.addPayment')}
              </button>
            </div>
          </div>
        ...
        <div className="space-y-3">
          <div className="flex justify-between items-center">
            <span className={`text-sm ${isHistory ? 'text-emerald-800/80' : 'text-accent-600'}`}>{t('debts.totalPaid')}</span>
            <span
          ...
          {payments.length > 0 && (
            <div className="space-y-1.5">
              <button
                type="button"
                id={`debt-payments-trigger-${debt.id}`}
                aria-expanded={!!paymentsExpanded[debt.id]}
                aria-controls={`debt-payments-list-${debt.id}`}
                title={
                  paymentsExpanded[debt.id]
                    ? t('debts.hidePaymentList')
                    : t('debts.showPaymentList')
                }
                onClick={() =>
                  setPaymentsExpanded((prev) => ({
                    ...prev,
                    [debt.id]: !prev[debt.id],
                  }))
                }
                className={
                  isHistory
              ...
              >
                ...
                <svg
                  ...
                  } ${paymentsExpanded[debt.id] ? 'rotate-180' : ''}`}
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                  aria-hidden
              ...
              {paymentsExpanded[debt.id] && (
                <ul
                  id={`debt-payments-list-${debt.id}`}
                  role="region"
                  aria-labelledby={`debt-payments-trigger-${debt.id}`}
          ...
          <div>
            <div className="flex justify-between items-center mb-1">
              <span className={`text-xs ${isHistory ? 'text-emerald-800/70' : 'text-accent-500'}`}>{t('debts.progress')}</span>
          ...
          >
            <span className={`text-sm ${isHistory ? 'text-emerald-800/80' : 'text-accent-600'}`}>{t('debts.remaining')}</span>
            <span
              className={`text-sm font-semibold ${
                isPaidOff ? (isHistory ? 'text-emerald-700' : 'text-green-600') : 'text-accent-800'
              }`}
            >
        ...
        {!isMounted ? (
          ...
          <>
            {activeDebts.length > 0 && (
              <section className="space-y-3 sm:space-y-4" aria-labelledby="debts-active-heading">
                <h2 id="debts-active-heading" className="text-base sm:text-lg font-bold text-accent-900 tracking-tight">
                  {t('debts.activeDebtsSection')}
                </h2>
            ...
            {paidOffDebts.length > 0 && (
              <section
                ...
                aria-labelledby="debts-paid-heading"
              >
                <div className="mb-1">
                  <h2
                    id="debts-paid-heading"
                    className="text-base sm:text-lg font-bold text-emerald-900/90 tracking-tight"
                  >
                    {t('debts.paidOffHistorySection')}
                  </h2>
                  <p className="text-xs text-emerald-800/60 mt-1 max-w-xl">{t('debts.paidOffHistoryHint')}</p>
                </div>
                {paidOffDebts.map((debt) => renderDebtCard(debt, 'history'))}
              </section>
            )}
          </>
        )}
